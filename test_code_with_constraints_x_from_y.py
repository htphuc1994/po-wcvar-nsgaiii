...
class CustomRepair(Repair):
    def _do(self, problem, pop, **kwargs):
        X = pop
        X = X.reshape((X.shape[0], problem.n, problem.tau, 2))  # Reshape to (pop_size, 2, n, tau, binary_decision)
...
class PortfolioOptimizationProblem(Problem):
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        X = X.reshape((X.shape[0], self.n, self.tau, 2))
        ...
        constraints.append((y[:, :, self.tau-1, 0]).reshape(X.shape[0], -1))  # cannot buy
